...
pub enum ExitReason {
    ...
    /// The logical processor encountered an exception while attempting
    /// to call the double-fault handler and that exception
    /// did not itself cause a VM exit due to the exception bitmap.
    TripleFault = 2,

    /// An INIT signal arrived.
    INITSignal = 3,

    /// An attempt to access memory with a guest-physical address was
